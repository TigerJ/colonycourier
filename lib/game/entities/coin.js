...
EntityCoin = ig.Entity.extend({
	...
	init: function( x, y, settings ) {
		...
		// Add the animations
		this.addAnim( 'red', this.bounciness*.2, [0,2,4,6,4,2]);
		this.addAnim( 'red_collected', 2, [8]);
		this.addAnim( 'blue', this.bounciness*.2, [1,3,5,7,5,3]);
		this.addAnim( 'blue_collected', 2, [9]);
		this.addAnim( 'orange', this.bounciness*.2, [10,12,14,16,14,12]);
		this.addAnim( 'orange_collected', 2, [18]);
		this.addAnim( 'purple', this.bounciness*.2, [11,13,15,17,15,13]);
		this.addAnim( 'purple_collected', 2, [19]);
		var varientX = Math.floor((Math.random()*5)+5);
		var varientY = Math.floor((Math.random()*25)+50);
