...
EntityHud = ig.Entity.extend({
	...
	update: function() {
		...
		//if game var has 386 turbo enabled
		if(ig.game._386turbo==true && this.hasSpawned386==false)
		{
			this.hasSpawned386=true;
			ig.game.spawnEntity(Entity386TurboHover,this.pos.x,this.pos.y);//sloppy spawn coords
		}
		//testing gravity ability
		if(ig.input.pressed('ability1') && this.toggleAbility1==false && ig.game._386turboQty>0)
		{
			ig.game.spawnEntity(EntityAbilityToggle,this.pos.x,this.pos.y,{name:'ab1'});
			this.toggleAbility1=true;
			ig.game.getEntitiesByType(EntityPlayer)[0].gravityFactor=0;
			ig.game.getEntitiesByType(EntityPlayer)[0].vel.y=0;
			this.ability1Timer=new ig.Timer(2);
			ig.game._386turboQty--;
		}
		else if(ig.input.pressed('ability1') && this.toggleAbility1==true)
		{
			ig.game.getEntitiesByType(EntityPlayer)[0].gravityFactor=1;
		...
		//if the player's gravity is ever non-0 then turn off the ability
		if(ig.game.getEntitiesByType(EntityPlayer)[0].gravityFactor==1 && this.toggleAbility1==true)
		{
			var abilities = ig.game.getEntitiesByType(EntityAbilityToggle);
			var abilitiesLength = abilities.length;
			for(i=0;i<abilitiesLength;i++)
		...
		//if the time is up reset timer, turn off the ability
		if(this.ability1Timer)
		{
			if(this.ability1Timer.delta()>0)
			{
				...
				this.toggleAbility1=false;
				this.ability1Timer=null;
			}	
		}
		//if the player is in upward motion turn off the ability
		if(ig.game.getEntitiesByType(EntityPlayer)[0].vel.y<0 && this.toggleAbility1==true)
		{
			ig.game.getEntitiesByType(EntityPlayer)[0].gravityFactor=1;
			var abilities = ig.game.getEntitiesByType(EntityAbilityToggle);
			var abilitiesLength = abilities.length;
		...
		if(ig.game.Cavanagh==true && this.hasSpawnedCavanagh==false)
		{
			...
			ig.game.spawnEntity(EntityCavanagh,this.pos.x,this.pos.y);//sloppy spawn coords
		}
		...
		if(ig.input.pressed('ability3') && this.toggleAbility3==false)
		{
