...
EntityPlayer = ig.Entity.extend({
	...
	update: function() {
		...
		if( this.standing && ig.input.pressed('jump') ) {
			...
			ig.game.jump2_sfx.volume=0.3;
			//ig.game.jump2_sfx.play();
		}
		...
		//check for virtual floor
		if( !ig.game.collisionMap.getTile(this.pos.x + (!this.flip ? +1 : this.size.x -1), this.pos.y + this.size.y+1))
        {
            ig.game.spawnEntity(EntityVirtualFloorBlock,this.pos.x+ (!this.flip ? +1 : this.size.x-1),this.pos.y+this.size.y+1);
        }


		/*if(ig.input.state("shoot"))
		{
			this.currentAnim=this.anims.beginWeapon;
			if(this.currentAnim.frame==3)
			{
				this.currentAnim=this.anims.weaponIdle;
			}
		}
		else
		{
			this.anims.beginWeapon.rewind();
		}*/
	},
